...
class AmazonAddProductToCart(BasePage):
    ...
    def search_for_product(self, product):
        search_box = self.find_element(self.SEARCH_BOX_ID)
        search_box.clear()
        search_box.send_keys(product)
        search_box.submit()

    def choose_first_search_result(self):
        first_result = self.find_element(self.FIRST_RESULT_CSS_SELECTOR)
        first_result.click()

    def add_product_to_cart(self):
        add_to_cart_button = self.find_element(self.ADD_TO_CART_ID)
        add_to_cart_button.click()
        time.sleep(3)

    def get_cart_count(self):
        cart_count_element = self.find_element(self.CART_COUNT)
        cart_count = int(cart_count_element.text)
        return cart_count

    def check_product_in_cart(self, product):
        self.open_url(self.AMAZON_CART_LINK)
        cart_items = self.find_elements(self.CART_ITEM_CSS_SELECTOR)
